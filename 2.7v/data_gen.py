...
def split_train_test(x, y , test_ratio):
    ...
    assert len(x) == len(y), u'Model expects x and y shapes to be the same'
    ...
    if y_train.shape != train_shape or x_train.shape != train_shape or x_test.shape != test_shape or y_test.shape != test_shape:
        raise Exception(u'One of the conversion test/train shapes gone wrong')
...
def wrap_for_env(vec, val):
    res = np.array([val])
    return np.lib.pad(res, ( 0, vec.shape[0] - 1 ), u'constant', constant_values=(0))
...
def np_corrupt(vec):
    return list(imap(lambda x: x+random.randint(0, 255)/random.randint(1,10) , vec))
...
class OpsEnv(object):
    ...
    def apply_op(self, selections, selections_mem, prev_sel, prev_sel_mem, inptX, batchX, batchY):
            ...
            error_flat[error_flat>0] = 2*self.cfg[u'max_reward']
            #print(error_flat)
            #print("math_error")
            #print(math_error)
            #print("erro_")
