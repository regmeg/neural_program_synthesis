    ...
    def __init__(self, cfg, ops_env, mem):
        ...
        with tf.name_scope(u"RNN_op"):
            ...
            with tf.name_scope(u"Params"):
                ...
                self.params[u"W2"] = tf.get_variable(u"W2", shape=[ cfg[u'state_size'], ops_env.num_of_ops ], dtype=cfg[u'datatype'], initializer=tf.contrib.layers.variance_scaling_initializer(factor=2.0,  mode='FAN_IN', uniform=False,  seed=None,  dtype=cfg['datatype']))
                self.params[u"b2"] = tf.Variable(np.zeros((ops_env.num_of_ops)), dtype=cfg[u'datatype'], name=u"b2")
                
                self.params[u"W3"] = tf.get_variable(u"W3", shape=[ cfg[u'num_features'], cfg[u'num_features'] ], dtype=cfg[u'datatype'], initializer=tf.contrib.layers.variance_scaling_initializer(factor=2.0,  mode='FAN_IN', uniform=False,  seed=None,  dtype=cfg['datatype']))
