...
class Operations(object):
    ...
    def tf_add(self, inpt, mem_sel=None):
        with tf.name_scope(u"tf_add"):
            result = tf.reduce_sum(inpt, axis = 1, name = u"add")
            reshape = tf.reshape(result, [self.cfg[u'batch_size'], -1], name = u"reshape")
            pad_res = tf.pad(reshape, [[0,0],[0,self.cfg[u'num_features'] - 1]], u"CONSTANT", name=u"pad")
            masked_ones = self.clean_infs(pad_res)
            return  masked_ones
    
    def tf_multiply(self ,inpt, mem_sel=None):
        with tf.name_scope(u"tf_multiply"):
            result = tf.reduce_prod(inpt, axis = 1, name = u"mult")
            reshape = tf.reshape(result , [self.cfg[u'batch_size'], -1], name = u"reshape")
            pad_res = tf.pad(reshape, [[0,0],[0,self.cfg[u'num_features'] - 1]], u"CONSTANT", name=u"pad")
            masked_ones = self.clean_infs(pad_res)
            return masked_ones
    ...
    def tf_stall(self, inpt, mem_sel=None):
        with tf.name_scope(u"tf_stall"):
            reshape = tf.reshape(inpt , [self.cfg[u'batch_size'], self.cfg[u'num_features']], name = u"reshape")
            return  reshape
    ...
    '''
    def tf_inpt_len(self,inpt, mem_sel=None):
        with tf.name_scope(u"tf_inpt_len"):
            feat = tf.constant(np.full((self.cfg[u'batch_size'], 1), self.cfg[u'num_features']), dtype=self.cfg[u'datatype'], name=u"num_fe_const")
            pad_res = tf.pad(feat, [[0,0],[0,self.cfg[u'num_features'] - 1]], u"CONSTANT", name=u"pad")
            return  pad_res
    ...
    ''' 
    def tf_divide(self, inpt, mem_sel=None):
        with tf.name_scope(u"tf_divide"):
            inpt_slice = tf.slice(inpt, [0,0], [self.cfg[u'batch_size'],1], name=u"inpt_slice")
            mem_slice = tf.slice(mem_sel, [0,0], [self.cfg[u'batch_size'],1], name=u"mem_slice")
    ...
    def tf_sub(self, inpt, mem_sel=None):
        with tf.name_scope(u"tf_sub"):
            inpt_slice = tf.slice(inpt, [0,0], [self.cfg[u'batch_size'],1], name=u"mem_slice")
            result = tf.subtract(mem_sel,  inpt_slice,  name=u"sub_inpt_mem_slice")
            reshape = tf.reshape(result , [self.cfg[u'batch_size'], -1], name = u"reshape")
            return reshape
    ...
    def not_zero(self, inpt, mem_sel=None):
        with tf.name_scope(u"not_zero"):
            greater = tf.greater(inpt,tf.zeros_like(inpt, dtype=self.cfg[u'datatype']))
            less = tf.less(inpt, tf.zeros_like(inpt, dtype=self.cfg[u'datatype']))
    ...
    def clean_infs(self,inpt, mem_sel=None):
        with tf.name_scope(u"clean_infs"):
            clean = tf.where(tf.is_inf(inpt), tf.ones_like(inpt, dtype=self.cfg[u'datatype']), inpt, name=u"clean")
            return clean
        
    def clean_nans(self,inpt, mem_sel=None):
        with tf.name_scope(u"clean_nans"):
            clean = tf.where(tf.is_nan(inpt), tf.zeros_like(inpt, dtype=self.cfg[u'datatype']), inpt, name=u"clean")
            return clean
        
    def tf_input_mem_concat(self, inpt, mem_sel=None):
        with tf.name_scope(u"tf_input_mem_concat"):
            #inpt_slice = tf.slice(inpt, [0,0], [self.cfg['batch_size'],1], name="tf_inp_mem_cnt_slice1")
            mem_slice = tf.slice(mem_sel, [0,0], [self.cfg[u'batch_size'],1], name=u"mem_slice")
            pad_res = tf.pad(mem_slice, [[0,0],[1,self.cfg[u'num_features'] - 2]], u"CONSTANT", name=u"pad")
            return  tf.add(inpt, pad_res, name=u"add")
