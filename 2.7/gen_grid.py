...
if FLAGS.type == u"RNN":
    ...
    params[u'relaunch'] = [False]

elif FLAGS.type == u"RL":
    #cfg for RL models
    params=OrderedDict()
    params[u'total_num_epochs'] = [80000]
    params[u'state_size'] = [200 for _ in range(FLAGS.amount)]
    params[u'test_ratio'] = [0.33]
    params[u'num_samples'] = [1500]
    params[u'batch_size']  = [100]
...
subprocess.Popen("echo '#!/bin/bash' > run_jobs"+FLAGS.type, shell=True, stderr=subprocess.STDOUT)
#subprocess.Popen("rm run_jobs", shell=True, stderr=subprocess.STDOUT)
for ind,cmd in enumerate(cmds):
    #print u"Lnch[" + unicode(ind+1) +u"]: " + cmd
    subprocess.Popen("echo " + cmd +" >> run_jobs"+FLAGS.type, shell=True, stderr=subprocess.STDOUT)
