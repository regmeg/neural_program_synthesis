...
class Operations:
    ...
    def tf_inpt_len(self,inpt, mem_sel=None):
        inpt = tf.assign(inpt, np.ones(inpt.get_shape()), name="tf_inpt_len_assign_ones")
        result = tf.reduce_sum(inpt, axis = 1, name = "tf_inpt_len_red")
        reshape = tf.reshape(result, [self.batch_size, -1], name = "tf_inpt_len_reshape")
        pad_res = tf.pad(reshape, [[0,0],[0,self.num_features - 1]], "CONSTANT", name="tf_inpt_len_pad")
        return  pad_res
    ...
    def tf_input_mem_concat(self, inpt, mem_sel=None):
        inpt_slice = tf.slice(inpt, [0,0], [self.batch_size,1], name="tf_inp_mem_cnt_slice1")
        mem_slice = tf.slice(mem_sel, [0,0], [self.batch_size,1], name="tf_inp_mem_cnt_slice1")
        intp_mem_concat  = tf.concat([inpt_slice, mem_slice],1, name="tf_inp_mem_cnt_cnct")
        pad_res = tf.pad(intp_mem_concat, [[0,0],[0,self.num_features - 2]], "CONSTANT", name="tf_inp_mem_cnt_pad")
        return  inpt
