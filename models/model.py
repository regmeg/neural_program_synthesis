...
for timestep in range(max_output_ops):
    ...
    softmax = tf.nn.softmax(logits*10, name="get_softmax")
...
train_step = tf.train.AdamOptimizer(0.1, name="AdamOpt").apply_gradients(zip(grads, [W,b,W2,b2]), name="min_loss")
print("grads are")
print(grads)
...
with tf.Session() as sess:
    ...
    for epoch_idx in range(num_epochs):
        ...
        for batch_idx in range(num_batches):
            ...
            _total_loss, _train_step, _current_state, _outputs, _grads = sess.run(
                    [total_loss, train_step, current_state, outputs, grads],
                    feed_dict={
